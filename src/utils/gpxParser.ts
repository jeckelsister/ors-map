  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        reject(new Error(`Erreur lors du parsing du fichier GPX: ${error instanceof Error ? error.message : 'Erreur inconnue'}`));
      }
    };
 ...
 */
export function convertGPXToWaypoints(gpxResult: GPXParseResult): Coordinates[] {
  const waypoints: Coordinates[] = [];

  // Priorité : routes > tracks > waypoints individuels
  if (gpxResult.routes.length > 0) {
    // Utiliser la première route
...
function extractMetadata(gpxElement: Element) {
  ...
  return {
    ...
    time: metadata.querySelector('time')?.textContent ? 
          new Date(metadata.querySelector('time')!.textContent!) : undefined
  };
}
...
function parseTrack(trkElement: Element, defaultId: string): GPXTrack {
  const name = trkElement.querySelector('name')?.textContent || `Track ${defaultId}`;
  ...
  Array.from(trkSegments).forEach((seg, segIndex) => {
    ...
    Array.from(trkPts).forEach((pt, ptIndex) => {
      waypoints.push(parseWaypoint(pt as Element, `${defaultId}-seg${segIndex}-pt${ptIndex}`));
    });
  });
...
function parseRoute(rteElement: Element, defaultId: string): GPXTrack {
  const name = rteElement.querySelector('name')?.textContent || `Route ${defaultId}`;
...
function simplifyTrackPoints(waypoints: Coordinates[], maxPoints: number = 20): Coordinates[] {
